...
class cronenvioN extends MY_Controller {
    ...
    public function index() {
        ...
        else
        {
            ...
            if($data["cron"][0]["estado"] == 1)
            {
                ...
                $this->registraLog('cronenvioN', 'cronenvioProcess', $nuevo);
            }    
            else
            {
                $this->cronenvioIn($data["cron"][0]["id"]);
                    ...
                    while($contador < 55)
                    {
                        ...
                        if ($tam == 0)
                        {
                            echo $contador." - NO HAY MENSAJES PARA EL CANAL N<br>";
                        ...
                        else 
                        {
                            ...
                            $url = $data["smpp"][0]["host"];
                            $smppport = $data["smpp"][0]["port"];

                            $username = $data["smpp"][0]["usr"];
                            ...
                            $system_type = $data["smpp"][0]["systemtype"];
                                ...
                                for ($i = 0; $i < $tam; $i++) {
                                    ...
                                    //inserta el registro
                                        //$username = 'SMSdobleviaCL1';
                                        //$password = 'Soporte2017';

                                        $logueo = "Basic " . base64_encode($username . ":" . $password);
                                ...
                                curl_setopt_array($curl, array(
                                  ...
                                  CURLOPT_POSTFIELDS => "{ \"from\":\"ContactoSMS\", \"to\":[ \"57".$data["pendientes"][$i]["numero"]."\" ], \"text\":\"".$data["pendientes"][$i]["mensaje"]."\" }",
                                  CURLOPT_HTTPHEADER => array(
                                    "accept: application/json",
                                    "authorization: ".$logueo."",
                               ...
                               $rta = json_decode($response,true);
                                    //3. validar exito o error
                                    if ($rta["messages"][0]["messageId"]) {
                                        /**
                                         ...
                                         * - Resumenes
                                         */
                                        $this->actualizar($data["pendientes"][$i]["id"],$data["pendientes"][$i]["idbase"],$data["pendientes"][$i]["idcarrie"],$data["pendientes"][$i]["numero"],$data["pendientes"][$i]["mensaje"],$data["pendientes"][$i]["nota"],$data["pendientes"][$i]["idcarrie"],$data["pendientes"][$i]["cargue"],'N',$data["pendientes"][$i]["fechacargue"],$rta["messages"][0]["messageId"]);
                                    }
    ...
    function actualizar($id,$idbase,$idcarrie,$numero,$mensaje,$nota,$carrie,$cargue,$canal,$fcargue,$rta) {
            /**
             * actualiza la tabla de registros  con el mensaje del smpp, actualiza el estado
             */
