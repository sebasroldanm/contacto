...
class cronenvioU extends MY_Controller {
    ...
    public function index() {
        ...
        else
        {
            ...
            {
                if( $data["cron"][0]["consulta"] > 30 OR $data["cron"][0]["consulta"] == null)
                {
                ...
                $this->registraLog('cronenvioU', 'cronenvioProcess', $nuevo);
            }    
            else
            {
                $this->cronenvioIn($data["cron"][0]["id"]);
                    ...
                    while($contador < 55)
                    {
                        ...
                        if ($tam == 0)
                        {
                            echo $contador." - NO HAY MENSAJES PARA EL CANAL U<br>";
                        ...
                        else 
                        {
                                ...
                                for ($i = 0; $i < $tam; $i++) {
                                    ...
                                    echo "<br>".$i . " - enviando mensaje " . $data["pendientes"][$i]["numero"] . " " . $data["pendientes"][$i]["mensaje"] ."-".$data["pendientes"][$i]["id"]. "  <br>\n";
                                    $datos = 0;
                                   
                                    $datos_my["sender"] = $from;
                                    $datos_my["recipient"] = '57'.$data["pendientes"][$i]["numero"];
                                    $datos_my["message"] = $data["pendientes"][$i]["mensaje"];
                                    $datos_my["status"] = 0;
                                    
                                    //inserta el registro
                                     $resultado =  $this->AdministradorModel->insert('sms_queue4', $datos_my, null,'mysql');
                                    ...
                                    if ($resultado) {
                                         ...
                                         * - Resumenes
                                         */
                                        $this->actualizar($data["pendientes"][$i]["id"],$data["pendientes"][$i]["idbase"],$data["pendientes"][$i]["idcarrie"],$data["pendientes"][$i]["numero"],$data["pendientes"][$i]["mensaje"],$data["pendientes"][$i]["nota"],$data["pendientes"][$i]["idcarrie"],$data["pendientes"][$i]["cargue"],'U',$data["pendientes"][$i]["fechacargue"]);
                                    }
