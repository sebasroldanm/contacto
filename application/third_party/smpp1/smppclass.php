...
//			$this->debug("Error: sender id too long.\n");
			return $errror["SetSender"]="Error: sender id too long.";
		}
...
//			$this->debug("Error was: $errstr.\n");
			return $error["_socket"]="Error opening SMPP session,Error was: $errstr";
		}
...
//			$this->debug("Error binding to SMPP server. Invalid credentials?\n");
                        return $error["credentials"]="Error binding to SMPP server. Invalid credentials?";
		}
		return ($status == 0);
	}
	...
	*/
	...
	{
		if (strlen($to) > 20) {
                        return $error["Send"]="to-address too long";
//			$this->debug("to-address too long.\n");
//			return;
		}
		if (!$this->_socket) {
                    return $error["socket"]="Not connected, while trying to send SUBMIT_SM";
//			$this->debug("Not connected, while trying to send SUBMIT_SM.\n");
		...
		while (list(, $part) = each($multi)) {
			...
			if ($status != 0) {
				return $error["SMPP_server"]="SMPP server returned error $status.\n";
//				$result = false;
			}
		}
...
//		return $result;
	...
	*/
	function End()
	{
		...
		if ($status != 0) {
                        return $error["SMPP_end"]="SMPP server returned error $status";
...
//				$this->debug("Connection lost.\n");
				...
				return $error["Connection"]='Connection lost';
			}
